"""GWen image-only VLM projector pretraining."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="GWen VLM projector pretrain")
    ...
    parser.add_argument("--text_checkpoint", required=True)
    parser.add_argument("--data_path", required=True)
    parser.add_argument("--image_root", default="")
    parser.add_argument("--out_dir", default="./out")
    parser.add_argument("--max_seq_len", type=int, default=450)
...
def main():
    ...
    logger = Logger(os.path.join(args.out_dir, "vlm_pretrain.log"), enabled=env["is_main"])

    tokenizer = load_tokenizer(args.tokenizer_path)
    text_ckpt = load_checkpoint(args.text_checkpoint, map_location="cpu")
    ...
    load_info = load_model_weights(model, text_ckpt, env["device"], strict=False)
    ...
    if env["is_main"]:
        logger.info(f"Loaded text checkpoint: missing={missing} unexpected={unexpected} skipped_shape={skipped}")
    if (missing or unexpected or skipped) and not args.allow_partial_load:
        raise RuntimeError(
            "Text checkpoint did not fully match the VLM pretrain model. "
            "Use the same tokenizer/config as text pretrain, or pass --allow_partial_load only for experiments."
        )
    ...
    logger.banner(
        f"GWen VLM projector pretrain: total={stats['total']/1e6:.1f}M "
        f"trainable={unwrap_model(model).get_num_trainable_params()/1e6:.2f}M dtype={dtype_name}"
    )
    model.train()
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(loader):
            ...
            if env["is_main"] and (current_batch % args.log_interval == 0 or current_batch == 1):
                ...
                logger.info(
                    f"VLM Pretrain Epoch [{epoch+1}/{args.epochs}] ({current_batch}/{iter_per_epoch}) "
    ...
    if env["is_main"]:
        final_path = os.path.join(args.out_dir, "vlm_pretrain_final.pth")
