...
| --- | ---: | ---: | ---: | ---: | --- | ---: | --- |
| `gwen8k_hybrid` | 8192 | 1024 | 16 | 8 / 2 KV | 3 GDN + 1 Full | ~328M | 当前主线 |
...
以下为历史旧配置实验记录。当前代码只保留 `gwen8k_hybrid` 主线配置。

Pretrain 使用 `dataset/pretrain_t2t_mini.jsonl`，SFT 使用 `dataset/sft_t2t_mini.jsonl`。历史三组模型均开启 `sigmoid` Gate Attention。
...
其中 A/B/C 为历史旧配置记录，当前代码不再提供 B/C 配置。
